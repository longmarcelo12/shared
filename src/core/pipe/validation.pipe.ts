...
export class ValidationPipe implements PipeTransform<any> {
  async transform(value, metadata: ArgumentMetadata) {
    ...
    if (errors.length > 0) {
      const message = await this.getMessage(errors);
  ...
  private async getMessage(errors: ValidationError[]): Promise<string> {
    ...
    if (!error.children || !error.children.length) {
      ...
      const mess = replacements.property
        .replace(/(([a-z])(?=[A-Z][a-zA-Z])|([A-Z])(?=[A-Z][a-z]))/g, '$1_')
        .toUpperCase();

      return `${mess}_${patternFinded}`;
    }
  }
